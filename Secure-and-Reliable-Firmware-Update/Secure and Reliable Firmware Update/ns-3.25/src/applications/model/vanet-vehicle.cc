...
NS_LOG_COMPONENT_DEFINE ("VANETVehicle");
NS_OBJECT_ENSURE_REGISTERED (VANETVehicle);

TypeId 
VANETVehicle::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::VANETVehicle")
    .SetParent<Application> ()
    .AddConstructor<VANETVehicle> ()
    .AddAttribute ("Local", "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&VANETVehicle::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("Remote", "The Address of the destination.",
                   AddressValue (),
                   MakeAddressAccessor (&VANETVehicle::m_target),
                   MakeAddressChecker ())
    .AddAttribute ("UDPTargetAddress", "The Address of the destination.",
                   AddressValue (),
                   MakeAddressAccessor (&VANETVehicle::m_UDPTarget),
                   MakeAddressChecker ())
    .AddAttribute ("NumberOfNodes", "The default size of packets received",
                   UintegerValue (25),
                   MakeUintegerAccessor (&VANETVehicle::m_nNodes),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Protocol", "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&VANETVehicle::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("DefaultRxSize", "The default size of packets received",
                   UintegerValue (512),
                   MakeUintegerAccessor (&VANETVehicle::m_defSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Mode", "The default size of packets received",
                   UintegerValue (1),
                   MakeUintegerAccessor (&VANETVehicle::m_mode),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Scenario", "The default size of packets received",
                   UintegerValue (1),
                   MakeUintegerAccessor (&VANETVehicle::m_scenario),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("PacketSize", "The default size of packets to be sent",
                   UintegerValue (512),
                   MakeUintegerAccessor (&VANETVehicle::m_pktSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("MeterType", "[0]->GW, [1]->AGG, [2]->LEAF",
                   UintegerValue (1),
                   MakeUintegerAccessor (&VANETVehicle::m_meterType),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Delay", "The default size of packets received",
                   UintegerValue (0),
                   MakeUintegerAccessor (&VANETVehicle::m_procDelay),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Child", "The number of child meters of this meter",
                   UintegerValue (1),
                   MakeUintegerAccessor (&VANETVehicle::m_childNum),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("LeafMeters", "The number of child meters of this meter",
                   UintegerValue (1),
                   MakeUintegerAccessor (&VANETVehicle::m_leafMeters),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FileName", "The output filename",
                   StringValue ("roundstat"),
                   MakeStringAccessor (&VANETVehicle::m_outputFilename),
                   MakeStringChecker ())
    .AddAttribute ("OperationIdentifier", "The identifier for the operation",
                    UintegerValue (0),
                    MakeUintegerAccessor (&VANETVehicle::m_operationId),
                    MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Port", "Port on which we listen for incoming packets.",
                   UintegerValue (10),
                   MakeUintegerAccessor (&VANETVehicle::m_UDPPort),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("MultipleTargets", "Whether there are multiple targets.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&VANETVehicle::m_multTargetFlag),
                   MakeUintegerChecker<uint8_t> ())
    .AddTraceSource ("Rx", "A packet has been received",
                     MakeTraceSourceAccessor (&VANETVehicle::m_rxTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("Tx", "A new packet is created and is sent",
                     MakeTraceSourceAccessor (&VANETVehicle::m_txTrace),
                     "ns3::Packet::TracedCallback")
  ;
  return tid;
}

VANETVehicle::VANETVehicle ()
{
...
VANETVehicle::VANETVehicle (uint16_t port, Address local, uint32_t delay)
{
  NS_LOG_FUNCTION (this);
  m_targetSocket = 0;
  m_UDPTargetSocket = 0;
...
uint32_t VANETVehicle::GetTotalRx () const
{
  NS_LOG_FUNCTION (this);
  return m_totalRx;
}
...
Ptr<Socket>
VANETVehicle::GetListeningSocket (void) const
{
  NS_LOG_FUNCTION (this);
  return m_socket;
}
...
std::list<Ptr<Socket> >
VANETVehicle::GetAcceptedSockets (void) const
{
  NS_LOG_FUNCTION (this);
  return m_socketList;
}

void VANETVehicle::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_socket = 0;
  m_targetSocket = 0;
...
// Application Methods
void VANETVehicle::StartApplication ()    // Called at time specified by Start
{
  NS_LOG_FUNCTION (this);
  ...
  m_UDPsocket->SetRecvCallback (MakeCallback (&VANETVehicle::HandleReadUDP, this));
  m_UDPsocket6->SetRecvCallback (MakeCallback (&VANETVehicle::HandleReadUDP, this));
...
//                MakeCallback (&VANETVehicle::HandleAccept, this));
//            m_localTCPsocket->SetCloseCallbacks (
//                MakeCallback (&VANETVehicle::HandlePeerClose, this),
//                MakeCallback (&VANETVehicle::HandlePeerError, this));
//
//            m_localTCPsocket->SetRecvCallback (MakeCallback (&VANETVehicle::HandleReadTCP, this));
//        }
//    }
...
////                            MakeCallback (&VANETVehicle::ConnectionSucceeded, this),
////                            MakeCallback (&VANETVehicle::ConnectionFailed, this));
////
////                socket->SetRecvCallback (MakeCallback (&VANETVehicle::HandleReadTCP, this));
////
...
//                            MakeCallback (&VANETVehicle::ConnectionFailed, this));
//                
//                m_socket->SetRecvCallback (MakeCallback (&VANETVehicle::HandleReadTCP, this));
//            }
////        }
//    }
//
...
void VANETVehicle::StopApplication ()     // Called at time specified by Stop
{
  NS_LOG_FUNCTION (this);
...
//  else
//    {
//      NS_LOG_WARN ("VANETVehicle found null socket to close in StopApplication");
//    }
//  
  ///////////////////////////////UDP////////////////////////////////////
  if (m_UDPsocket != 0) 
...
void VANETVehicle::CancelEvents ()
{
  NS_LOG_FUNCTION (this);
...
void VANETVehicle::HandleReadTCP (Ptr<Socket> socket)
{
  ...
  while ((packet = socket->RecvFrom (from)))
  {
      ...
      //[0]->GW, [1]->AGG, [2]->LEAF
        ...
        if(m_meterType == (uint32_t)0){
            ...
            Simulator::Schedule (NanoSeconds (m_procDelay), &VANETVehicle::SendToTargetSocket, this, packet, socket);
        }
            ...
            Simulator::ScheduleNow (&VANETVehicle::EchoPacket, this, packet);
        }
        else{
...
void VANETVehicle::HandleReadUDP (Ptr<Socket> socket)
{
    NS_LOG_FUNCTION (this << socket);
...
//                        "s EV (Node " << GetNode()->GetId() << 
//                        ") received " << packet->GetSize () << " bytes sent at " << 
//                        txtime.GetSeconds() << " from " <<
//                        InetSocketAddress::ConvertFrom (from).GetIpv4 () << " port " <<
//                        InetSocketAddress::ConvertFrom (from).GetPort ());
//        }
//        else if (Inet6SocketAddress::IsMatchingType (from))
...
//        }
        ...
        if(seqNum == (uint32_t)0){
            ...
            NS_LOG_INFO("At time " << now.GetSeconds () << "s EV (Node " << GetNode()->GetId() << 
            ...
            NS_LOG_INFO("At time " << now.GetSeconds () << "s An EV cannot receive a connection request!!!");
        }
        else{
            NS_LOG_INFO("At time " << now.GetSeconds () << "s EV (Node " << GetNode()->GetId() << 
                        ") received a connection reply sent at " << txtime.GetSeconds() << 
                        " from " << InetSocketAddress::ConvertFrom (from).GetIpv4 () << " port " <<
                        InetSocketAddress::ConvertFrom (from).GetPort () << 
...
void VANETVehicle::EchoPacket(Ptr<Packet> packet){
...
void VANETVehicle::SendToTarget(Ptr<Packet> packet){
...
void VANETVehicle::SendToTargetSocket(Ptr<Packet> packet, Ptr<Socket> socket){
    NS_LOG_LOGIC ("Send the Request to the target");
...
void VANETVehicle::HandleReport(Ptr<Packet> packet, Address from) 
{
   SeqTsHeader seqTs;
   uint32_t m_rxBytes;
   ...
   if(senderIP.IsEqual(Ipv4Address ("10.1.2.1"))){
       if(m_scenario == (uint32_t)1)
           m_sendEvent = Simulator::Schedule (NanoSeconds(0.0), &VANETVehicle::SendPacket, this, seqNum);
   }
   else if(senderIP.IsEqual(Ipv4Address ("10.1.1.1"))){
       if(m_scenario == (uint32_t)1)
           m_sendEvent = Simulator::Schedule (NanoSeconds(116828508.0), &VANETVehicle::SendPacket, this, seqNum);
   }
...
//                    m_sendEvent = Simulator::Schedule (NanoSeconds(m_procDelay), &VANETVehicle::SendPacket, this);
//                }
//            }
//            else{
//                uint32_t seqNumCounter = itSeq->second;
...
void VANETVehicle::SendPacket (uint32_t seqNum)
{
  NS_LOG_FUNCTION (this);
 // Create the socket if not already
...
void VANETVehicle::SendUDPPacket ()
{
  NS_LOG_FUNCTION (this);
 // Create the socket if not already
...
void VANETVehicle::ConnectionSucceeded (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void VANETVehicle::ConnectionFailed (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  NS_LOG_INFO("Connection Failed");
}

bool VANETVehicle::IsThereAnyPreviousStoredPacket(Address from) {
...
void VANETVehicle::ReportStat (std::ostream & os)  
{
...
void VANETVehicle::HandlePeerClose (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
}
 
void VANETVehicle::HandlePeerError (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
}
 

void VANETVehicle::HandleAccept (Ptr<Socket> s, const Address& from)
{
  NS_LOG_FUNCTION (this << s << from);
  s->SetRecvCallback (MakeCallback (&VANETVehicle::HandleReadTCP, this));
