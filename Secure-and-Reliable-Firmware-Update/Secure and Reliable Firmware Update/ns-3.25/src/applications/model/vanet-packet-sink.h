...
private:
  ...
  void HandleReadEV (Ptr<Socket> socket);
  void EchoPacket(Ptr<Packet>);
  void SendToTarget(Ptr<Packet>);
  void SendToTargetSocket(Ptr<Packet> packet, Ptr<Socket> socket);
  void HandleAcceptEV (Ptr<Socket>, const Address& from);
  void HandleAcceptMesh (Ptr<Socket>, const Address& from);
  void HandlePeerClose (Ptr<Socket>);
  void HandlePeerError (Ptr<Socket>);
  void HandleReport (Ptr<Packet> pkt, Address from, Ptr<Socket> socket);
  void HandleReportEV(Ptr<Packet> packet, Address from, Ptr<Socket> socket);
  ...
  std::vector<DataWaitingPacket> m_waitingPacketEV;
  std::vector<StatRecord> m_stat;
  std::vector<Address> m_targets;
  
  typedef std::map< uint32_t, std::pair<Ptr<Socket>, Address> > MeterSeqNumMap;
  MeterSeqNumMap m_SocketAddressMap;

  Address         m_local;        // Local address to bind to
  Address         m_localEV;        // Local address to bind to
