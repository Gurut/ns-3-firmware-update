...
NS_LOG_COMPONENT_DEFINE ("MeshVANETMeter");
NS_OBJECT_ENSURE_REGISTERED (MeshVANETMeter);

TypeId 
MeshVANETMeter::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MeshVANETMeter")
    .SetParent<Application> ()
    .AddConstructor<MeshVANETMeter> ()
    .AddAttribute ("Local", "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&MeshVANETMeter::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("FirstSendingTime",
                   "The time to wait between packets", TimeValue (Seconds (50.0)),
                   MakeTimeAccessor (&MeshVANETMeter::m_firstSendingTime),
                   MakeTimeChecker ())
    .AddAttribute ("ServiceInterval",
                   "The time to wait between packets", TimeValue (Seconds (3.0)),
                   MakeTimeAccessor (&MeshVANETMeter::m_serviceMessageInterval),
                   MakeTimeChecker ())
    .AddAttribute ("Remote", "The Address of the destination.",
                   AddressValue (),
                   MakeAddressAccessor (&MeshVANETMeter::m_target),
                   MakeAddressChecker ())
    .AddAttribute ("UDPTargetAddress", "The Address of the destination.",
                   AddressValue (),
                   MakeAddressAccessor (&MeshVANETMeter::m_UDPTarget),
                   MakeAddressChecker ())
    .AddAttribute ("NumberOfNodes", "The default size of packets received",
                   UintegerValue (25),
                   MakeUintegerAccessor (&MeshVANETMeter::m_nNodes),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Protocol", "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&MeshVANETMeter::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("DefaultRxSize", "The default size of packets received",
                   UintegerValue (512),
                   MakeUintegerAccessor (&MeshVANETMeter::m_defSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Mode", "The default size of packets received",
                   UintegerValue (1),
                   MakeUintegerAccessor (&MeshVANETMeter::m_mode),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Scenario", "The default size of packets received",
                   UintegerValue (1),
                   MakeUintegerAccessor (&MeshVANETMeter::m_scenario),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("PacketSize", "The default size of packets to be sent",
                   UintegerValue (512),
                   MakeUintegerAccessor (&MeshVANETMeter::m_pktSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("ServicePacketSize",
                   "Size of packets generated. The minimum packet size is 12 bytes which is the size of the header carrying the sequence number and the time stamp.",
                   UintegerValue (1024),
                   MakeUintegerAccessor (&MeshVANETMeter::m_serviceMessageSize),
                   MakeUintegerChecker<uint32_t> (12,1500))
    .AddAttribute ("MeterType", "[0]->GW, [1]->AGG, [2]->LEAF",
                   UintegerValue (1),
                   MakeUintegerAccessor (&MeshVANETMeter::m_meterType),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Delay", "The default size of packets received",
                   UintegerValue (0),
                   MakeUintegerAccessor (&MeshVANETMeter::m_procDelay),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Child", "The number of child meters of this meter",
                   UintegerValue (1),
                   MakeUintegerAccessor (&MeshVANETMeter::m_childNum),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("LeafMeters", "The number of child meters of this meter",
                   UintegerValue (1),
                   MakeUintegerAccessor (&MeshVANETMeter::m_leafMeters),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FileName", "The output filename",
                   StringValue ("roundstat"),
                   MakeStringAccessor (&MeshVANETMeter::m_outputFilename),
                   MakeStringChecker ())
    .AddAttribute ("OperationIdentifier", "The identifier for the operation",
                    UintegerValue (0),
                    MakeUintegerAccessor (&MeshVANETMeter::m_operationId),
                    MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Port", "Port on which we listen for incoming packets.",
                   UintegerValue (10),
                   MakeUintegerAccessor (&MeshVANETMeter::m_UDPPort),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("MultipleTargets", "Whether there are multiple targets.",
                   UintegerValue (0),
                   MakeUintegerAccessor (&MeshVANETMeter::m_multTargetFlag),
                   MakeUintegerChecker<uint8_t> ())
    .AddTraceSource ("Rx", "A packet has been received",
                     MakeTraceSourceAccessor (&MeshVANETMeter::m_rxTrace),
                     "ns3::Packet::TracedCallback")
    .AddTraceSource ("Tx", "A new packet is created and is sent",
                     MakeTraceSourceAccessor (&MeshVANETMeter::m_txTrace),
                     "ns3::Packet::TracedCallback")
  ;
  return tid;
}

MeshVANETMeter::MeshVANETMeter ()
{
...
MeshVANETMeter::MeshVANETMeter (uint16_t port, Address local, uint32_t delay)
{
  NS_LOG_FUNCTION (this);
  m_targetSocket = 0;
  m_UDPTargetSocket = 0;
...
uint32_t MeshVANETMeter::GetTotalRx () const
{
  NS_LOG_FUNCTION (this);
  return m_totalRx;
}
...
Ptr<Socket>
MeshVANETMeter::GetListeningSocket (void) const
{
  NS_LOG_FUNCTION (this);
  return m_socket;
}
...
std::list<Ptr<Socket> >
MeshVANETMeter::GetAcceptedSockets (void) const
{
  NS_LOG_FUNCTION (this);
  return m_socketList;
}

void MeshVANETMeter::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  m_socket = 0;
  m_targetSocket = 0;
...
// Application Methods
void MeshVANETMeter::StartApplication ()    // Called at time specified by Start
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_UDPsocket == 0)
    {
      ...
      if (Ipv4Address::IsMatchingType(m_UDPTarget) == true)
        {
          m_UDPsocket->Bind ();
          m_UDPsocket->Connect (InetSocketAddress (Ipv4Address::ConvertFrom(m_UDPTarget), m_UDPPort));
        }
      else if (Ipv6Address::IsMatchingType(m_UDPTarget) == true)
        {
          m_UDPsocket->Bind6 ();
          m_UDPsocket->Connect (Inet6SocketAddress (Ipv6Address::ConvertFrom(m_UDPTarget), m_UDPPort));
        }
    }
...
//    }
  ...
  m_UDPsocket->SetRecvCallback (MakeCallback (&MeshVANETMeter::HandleReadUDP, this));
  m_UDPsocket->SetAllowBroadcast (true);
  m_sendEvent = Simulator::Schedule (m_firstSendingTime, &MeshVANETMeter::Send, this);
//  m_UDPsocket6->SetRecvCallback (MakeCallback (&MeshVANETMeter::HandleReadUDP, this));
...
////////////////////////////////////TCP/////////////////////////////////////////
    ...
    //        m_localTCPsocket->ShutdownSend ();
            ...
            m_localTCPsocket->SetAcceptCallback (
                ...
                MakeCallback (&MeshVANETMeter::HandleAccept, this));
            m_localTCPsocket->SetCloseCallbacks (
                MakeCallback (&MeshVANETMeter::HandlePeerClose, this),
                MakeCallback (&MeshVANETMeter::HandlePeerError, this));

            m_localTCPsocket->SetRecvCallback (MakeCallback (&MeshVANETMeter::HandleReadTCP, this));
        }
    }
...
//                            MakeCallback (&MeshVANETMeter::ConnectionSucceeded, this),
//                            MakeCallback (&MeshVANETMeter::ConnectionFailed, this));
//
//                socket->SetRecvCallback (MakeCallback (&MeshVANETMeter::HandleReadTCP, this));
//
...
//        else{
    ...
    //            m_socket->ShutdownRecv ();
                ...
                m_socket->SetRecvCallback (MakeCallback (&MeshVANETMeter::HandleReadTCP, this));
            }
//        }
    }
...
//  seqTs.SetSeq (m_sent);
  ...
  Ptr<Packet> p = Create<Packet> (m_serviceMessageSize-(8+4)); // 8+4 : the size of the seqTs header
  p->AddHeader (seqTs);

  std::stringstream peerAddressStringStream;
  if (Ipv4Address::IsMatchingType (m_UDPTarget))
    {
      peerAddressStringStream << Ipv4Address::ConvertFrom (m_UDPTarget);
    }
  else if (Ipv6Address::IsMatchingType (m_UDPTarget))
    {
      peerAddressStringStream << Ipv6Address::ConvertFrom (m_UDPTarget);
    }

  if ((m_UDPsocket->Send (p)) >= 0)
    {
//      NS_LOG_INFO ("A service message broadcasted! TraceDelay TX " << m_serviceMessageSize << " bytes to "
...
//                                    << (Simulator::Now ()).GetSeconds ());
    ...
    {
      NS_LOG_INFO ("Error while sending " << m_serviceMessageSize << " bytes to "
                                          << peerAddressStringStream.str ());
    }

  m_sendEvent = Simulator::Schedule (m_serviceMessageInterval, &MeshVANETMeter::Send, this);
}

void MeshVANETMeter::StopApplication ()     // Called at time specified by Stop
{
  NS_LOG_FUNCTION (this);
  ...
  if(m_targetSocket != 0)
    ...
    {
      NS_LOG_WARN ("MeshVANETMeter found null socket to close in StopApplication");
    }
...
void MeshVANETMeter::CancelEvents ()
{
  NS_LOG_FUNCTION (this);
...
void MeshVANETMeter::HandleReadTCP (Ptr<Socket> socket)
{
  ...
  while ((packet = socket->RecvFrom (from)))
  {
      ...
      //[0]->GW, [1]->AGG, [2]->LEAF
        ...
        if(m_meterType == (uint32_t)0){
            ...
            NS_LOG_INFO("GW meter replied another meter with a connection reply at " << Simulator::Now().GetSeconds() << "s!!!");
            
            Simulator::Schedule (NanoSeconds (m_procDelay), &MeshVANETMeter::SendToTargetSocket, this, packet, socket);
        }
        else if(m_meterType == (uint32_t)2){
            if (InetSocketAddress::IsMatchingType (from)){
                NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds () << 
                            "s Mesh VANET Meter (Node " << GetNode()->GetId() << 
            ...
            Simulator::ScheduleNow (&MeshVANETMeter::EchoPacket, this, packet);
        }
        else{
...
void MeshVANETMeter::HandleReadUDP (Ptr<Socket> socket)
{
    NS_LOG_FUNCTION (this << socket);
    ...
    while ((packet = socket->RecvFrom (from))){
        ...
        if(seqNum == (uint32_t)0){
//            SeqTsHeader header;
//            header.SetSeq((uint32_t)1);
//            Ptr<Packet> pkt = Create<Packet>(packet->GetSize());
//            pkt->AddHeader(header);
            
            NS_LOG_INFO("At time " << now.GetSeconds () << "s Mesh VANET Meter (Node " << GetNode()->GetId() << 
                        ") received a service message sent at " << txtime.GetSeconds() << 
                        " from " << InetSocketAddress::ConvertFrom (from).GetIpv4 () << " port " <<
                        InetSocketAddress::ConvertFrom (from).GetPort ());
//            
//            socket->SendTo (packet, 0, from);
        }
        else if(seqNum == (uint32_t)1){
            NS_LOG_INFO("At time " << now.GetSeconds () << "s A Mesh VANET Meter received a connection request!!!");
    ...
    //        Simulator::Schedule (Seconds(2.0), &MeshVANETMeter::EchoPacket, this, packet);
            if(m_meterType == (uint32_t)0)
                Simulator::Schedule (NanoSeconds (m_procDelay), &MeshVANETMeter::EchoPacket, this, packet);
            else
                Simulator::Schedule (NanoSeconds (m_procDelay), &MeshVANETMeter::SendToTarget, this, packet);
        }
        else{
            NS_LOG_INFO("A Mesh VANET Meter should not receive a connection reply message over a UDP connection!!!");
        }
        
//        if (InetSocketAddress::IsMatchingType (from))
//        {
//            NS_LOG_INFO ("At time " << now.GetSeconds () << 
...
void MeshVANETMeter::EchoPacket(Ptr<Packet> packet){
...
void MeshVANETMeter::SendToTarget(Ptr<Packet> packet){
...
void MeshVANETMeter::SendToTargetSocket(Ptr<Packet> packet, Ptr<Socket> socket){
    NS_LOG_LOGIC ("Send the Request to the target");
...
void MeshVANETMeter::HandleReport(Ptr<Packet> packet, Address from) 
{
   SeqTsHeader seqTs;
   uint32_t m_rxBytes;
   ...
   if(senderIP.IsEqual(Ipv4Address ("10.1.2.1"))){
       if(m_scenario == (uint32_t)1)
           m_sendEvent = Simulator::Schedule (NanoSeconds(0.0), &MeshVANETMeter::SendPacket, this, seqNum);
   }
   else if(senderIP.IsEqual(Ipv4Address ("10.1.1.1"))){
       if(m_scenario == (uint32_t)1)
           m_sendEvent = Simulator::Schedule (NanoSeconds(116828508.0), &MeshVANETMeter::SendPacket, this, seqNum);
   }
...
//                    m_sendEvent = Simulator::Schedule (NanoSeconds(m_procDelay), &MeshVANETMeter::SendPacket, this);
//                }
//            }
//            else{
//                uint32_t seqNumCounter = itSeq->second;
...
void MeshVANETMeter::SendPacket (uint32_t seqNum)
{
  NS_LOG_FUNCTION (this);
 // Create the socket if not already
...
void MeshVANETMeter::SendUDPPacket ()
{
  NS_LOG_FUNCTION (this);
 // Create the socket if not already
...
void MeshVANETMeter::ConnectionSucceeded (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
...
void MeshVANETMeter::ConnectionFailed (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  NS_LOG_INFO("Connection Failed");
}

bool MeshVANETMeter::IsThereAnyPreviousStoredPacket(Address from) {
...
void MeshVANETMeter::ReportStat (std::ostream & os)  
{
...
void MeshVANETMeter::HandlePeerClose (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
}
 
void MeshVANETMeter::HandlePeerError (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
}
 

void MeshVANETMeter::HandleAccept (Ptr<Socket> s, const Address& from)
{
  NS_LOG_FUNCTION (this << s << from);
  s->SetRecvCallback (MakeCallback (&MeshVANETMeter::HandleReadTCP, this));
