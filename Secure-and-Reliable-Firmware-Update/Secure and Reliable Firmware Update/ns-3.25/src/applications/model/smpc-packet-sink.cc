...
SmpcPacketSink::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SmpcPacketSink")
    ...
    .AddAttribute ("UDPTargetAddress", "The Address of the destination.",
                   AddressValue (),
                   MakeAddressAccessor (&SmpcPacketSink::m_UDPTarget),
                   MakeAddressChecker ())
    .AddAttribute ("NumberOfNodes", "The default size of packets received",
                   UintegerValue (25),
                   MakeUintegerAccessor (&SmpcPacketSink::m_nNodes),
    ...
    .AddAttribute ("Scenario", "The default size of packets received",
                   UintegerValue (1),
                   MakeUintegerAccessor (&SmpcPacketSink::m_scenario),
                   MakeUintegerChecker<uint32_t> ())
    ...
    .AddAttribute ("Port", "Port on which we listen for incoming packets.",
                   UintegerValue (9),
                   MakeUintegerAccessor (&SmpcPacketSink::m_UDPPort),
...
void SmpcPacketSink::StartApplication ()    // Called at time specified by Start
{
  ...
  m_UDPsocket->SetRecvCallback (MakeCallback (&SmpcPacketSink::HandleRead, this));
  m_UDPsocket6->SetRecvCallback (MakeCallback (&SmpcPacketSink::HandleRead, this));
...
void SmpcPacketSink::HandleReport(Ptr<Packet> packet, Address from) 
{
   ...
   if(senderIP.IsEqual(Ipv4Address ("10.1.2.1"))){
       if(m_scenario == (uint32_t)1)
           m_sendEvent = Simulator::Schedule (NanoSeconds(0.0), &SmpcPacketSink::SendPacket, this, seqNum);
   }
   else if(senderIP.IsEqual(Ipv4Address ("10.1.1.1"))){
       if(m_scenario == (uint32_t)1)
           m_sendEvent = Simulator::Schedule (NanoSeconds(116828508.0), &SmpcPacketSink::SendPacket, this, seqNum);
   }
...
void SmpcPacketSink::SendPacket (uint32_t seqNum)
{
  NS_LOG_FUNCTION (this);
 // Create the socket if not already
...
void SmpcPacketSink::SendUDPPacket ()
{
  NS_LOG_FUNCTION (this);
 // Create the socket if not already
